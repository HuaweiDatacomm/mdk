...
serialise:
  * module containing methods to serialise MDK class hierarchie
...
class YangDataSerialiser(object):
    ...
    def default(self, obj):
        ...
        # The value class is actually a MDK class, so map it
    ...
    def map_pyangbind_type(self, map_val, original_yang_type, obj):
        if map_val in ["MDK.lib.yangtypes.RestrictedClass", "RestrictedClassType"]:
            map_val = getattr(obj, "_restricted_class_base")[0]

        if map_val in ["MDK.lib.yangtypes.ReferencePathType", "ReferencePathType"]:
            return self.default(obj._get())
        elif map_val in ["MDK.lib.yangtypes.RestrictedPrecisionDecimal", "RestrictedPrecisionDecimal"]:
...
def make_generate_ietf_tree(yname_ns_func):
    """
    Convert a MDK class to a format which encodes to the IETF JSON
    ...
    def generate_ietf_tree_old(obj, parent_namespace=None, flt=False, with_defaults=None):
        ...
        if generated_by == "YANGListType":
            ...
            # MDK, so we simply serialise it how we would if it
            # were a scalar.
            return obj
...
class pybindIETFXMLDecoder(object):
    ...
    Use the `decode()` method to return an MDK representation of the yang object.
    """
    @staticmethod
